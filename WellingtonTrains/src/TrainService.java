...
//Obtain the stopping time of the train at the station based on the station (no stopping or no return -1
	public static int getStopTimeForStation(TrainService service, Station sta) {
		TrainLine line = service.getTrainLine();
	    List<Station> stations = line.getStations();
	    int index = stations.indexOf(sta);
	    if (index != -1 && index < service.getTimes().size()) {
	        return service.getTimes().get(index);
	    }
		return -1;
